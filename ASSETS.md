...
|---|---|---|
| `assets/characters/red/*.png` (85 strips) + `character.json` | PixelLab exports of Evan's own character: `NewRedShirtAnimations`, the `Man_with_a_deep_forest` and `NewAnims`/`ewlAnimationsNew` drops (sit, jump, screen-interact, guitar, seated-console sets), plus recoveries straight from PixelLab's CDN via the MCP `get_character` frame URLs (chair swivel, the two seated-console NE clips). The 17-frame `Sit and bring up console` NE take only existed at 172x164, so it was realigned onto 256x256 canvases by cross-correlating its final frame against the seated loop (96.5% pixel match at offset 42,46). | `tools/pack-character.mjs` (multi-root; later roots win): every 256x256 frame cropped to a single shared 180x181 alpha bounding box so the feet never drift between clips, then packed into one horizontal strip per animation and direction. Folders whose canvas is not the dominant 256px are reported and skipped. |
...
| `assets/audio/explosion-8bit.ogg`, `beam-charge.ogg`, `beam-fire.ogg` | Synthesized from scratch for this site (Node script: NES-style sample-and-hold noise burst with a sine thump; rising pulse-wave charge whine; detuned-sawtooth beam drone) | No source recordings involved. Peak-normalized to -6 dBFS, libvorbis q3. |
